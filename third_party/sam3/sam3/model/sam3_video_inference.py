...
class Sam3VideoInferenceWithInstanceInteractivity(Sam3VideoInference):
    ...
    def parse_action_history_for_propagation(self, inference_state):
        ...
        if len(action_history) == 0:
            ...
            return "propagation_full", None
        # If no propagation has ever run but we already have add/refine actions,
        # we must run a full propagation first to populate cached_frame_outputs.
        has_propagation = any("propagation" in action["type"] for action in action_history)
        if not has_propagation and not inference_state.get("cached_frame_outputs"):
            return "propagation_full", None
