...
class InferencePipeline:
    ...
    def sample_sparse_structure_multi_view(
        ...
        """
        Multi-view sparse structure sampling.
        """
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=self.shape_model_dtype):
                ...
                logger.info(
                    f"[Stage 1 Multi-view] Generated shape_latent shape: {shape_latent.shape}"
                )
                ss = ss_decoder(
                ...
                logger.info(
                    f"[Stage 1 Multi-view] Decoded sparse structure shape: {ss.shape}"
                )
                coords = torch.argwhere(ss > 0)[:, [0, 2, 3, 4]].int()
                logger.info(
                    f"[Stage 1 Multi-view] Extracted coords shape: {coords.shape}"
                )
                ...
                if self.downsample_ss_dist > 0:
                    coords = prune_sparse_structure(
                        coords, max_neighbor_axes_dist=self.downsample_ss_dist
                    )
    ...
    def sample_slat_multi_view(
        ...
        with torch.autocast(device_type="cuda", dtype=self.dtype):
            with torch.no_grad():
                ...
                slat = sp.SparseTensor(coords=coords, feats=slat[0]).to(device)
        ...
        for image, mask in zip(view_images, view_masks):
            if mask is not None:
                ...
                if image.shape[-1] == 3:
                    ...
                    rgba_image = np.concatenate([image[..., :3], mask], axis=-1).astype(
                        np.uint8
                    )
                else:
                    raise ValueError(f"Unexpected image shape: {image.shape}")
            else:
                rgba_image = np.array(image) if not isinstance(image, Image.Image) else np.array(image)
